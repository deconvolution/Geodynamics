using Revise, Printf, Plots, Enzyme
...
#=
    # Compute derivative of τxx vs εxx here, using Enzyme
    dτxx_dεxx=zeros(size(ηc));

    f1(ηc,εxx)=2*ηc*εxx;
    for i=1:size(ηc,1)
        for j=1:size(ηc,2)
            for k=1:size(ηc,3)
                dτxx_dεxx[i,j,k]= autodiff(f1,Active,
                (ηc[i,j,k]),Active(εxx[i,j,k]))[1];
            end
        end
    end
    p2 = heatmap(dτxx_dεxx[:, (size(τxy,2))÷2, :]')
    display(p2)
=#
    ∂τxx_∂εxx = ones(size(εxx))
    dexx = ones(size(εxx))
    autodiff(ComputeStress, Const,
      Duplicated(τxx,∂τxx_∂εxx), τyy, τzz, τxy, τxz, τyz,
      Duplicated(εxx, dexx), εyy, εzz, εxy, εxz, εyz,
      ηc, ηxy, ηxz, ηyz )3
    #ComputeStress( Duplicated(τxx,∂τxx_∂εxx), τyy, τzz, τxy, τxz, τyz, Active(εxx), εyy, εzz, εxy, εxz, εyz, ηc, ηxy, ηxz, ηyz )
        ...
        for j = 1:size(εxx,2)+1
            for i = 1:size(εxx,1)+1
                if (i<=size(εxy,1)) && (j<=size(εxy,2)) && (k<=size(εxy,3))
                    ...
                    # τxy[i,j,k]=autodiff(f1,Active,(ηxy[i,j,k]),Active(εxy[i,j,k]))[1];
                    τxy[i,j,k] = 2*ηxy[i,j,k]*εxy[i,j,k]
                end
                if (i<=size(εxz,1)) && (j<=size(εxz,2)) && (k<=size(εxz,3))
